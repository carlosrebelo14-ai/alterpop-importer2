...
export async function upsertCurationQueueBatchFromRecords(records = []) {
  if (!records.length) return;
  const queue = await loadCurationQueue();
  const bySku = new Map(queue.items.map((item, index) => [item.sku, { item, index }]));
  ...
  for (const record of records) {
    ...
    if (existingEntry) {
      ...
      if (existing.status === "APPROVED" || existing.status === "REJECTED" || existing.status === "PUBLISHED") {
        ...
        existing.shopifyStatus = shopifyStatus;
        existing.status = queueStatus;
        existing.metadata = { ...buildMetadata(record, [...elite.reasons, ...smart.reasons, ...gateReasons], titleEs), ...metaExtras };
      }
    } else {
      const newItem = {
        sku: record.sku,
