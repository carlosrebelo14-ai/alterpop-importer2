...
export async function insertCatalogProductBatch(shop, chunk, options = {}) {
  ...
  // 23 parâmetros por produto * 43 produtos = 989 (< limite de 999 do SQLite)
  const CHUNK = 43;
  for (let i = 0; i < unique.length; i += CHUNK) {
    ...
    const sql = `
      INSERT INTO CatalogProduct (shop, sku, title, categoryMain, categorySegments, vendor, vendorNorm, stock, franchises, franchiseRefs, resolvedFranchise, resolvedFranchiseLayer, resolvedLine, netPrice, grossPrice, imageUrl, barcode, weightKg, titleSource, dimensions, hsCode, originalTitle, cleanTitle, indexedAt)
