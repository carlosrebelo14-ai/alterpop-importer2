...
export async function insertCatalogProductBatch(shop, chunk, options = {}) {
  ...
  // rethrow:true — o worker fork (catalog-index-worker.mjs) tem catch próprio que
  // marca o ciclo "failed" com a mensagem real; deixar isto em silêncio significa
  // um ciclo de indexação a "terminar bem" sem ter escrito produto nenhum.
  await safePrisma(
    "catalogProduct.upsertBatch",
    () => prisma.$transaction(statements.map(({ sql, params }) => prisma.$executeRawUnsafe(sql, ...params))),
    { rethrow: true }
  );
...
export async function bulkInsertCatalogFilterTags(tags = []) {
  ...
  for (let i = 0; i < tags.length; i += CHUNK) {
    ...
    // rethrow:true — o .catch() interno já decidiu que isto não é um "Unique
    // constraint" tolerável antes de relançar; um safePrisma silencioso por cima
    // anula essa decisão.
    await safePrisma(
      "catalogProductFilterTag.createMany",
      () =>
