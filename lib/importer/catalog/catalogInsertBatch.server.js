...
export async function insertCatalogProductBatch(shop, chunk, options = {}) {
  ...
  // 24 parâmetros por produto * 41 produtos = 984 (< limite de 999 do SQLite)
  const CHUNK = 41;
  for (let i = 0; i < unique.length; i += CHUNK) {
    ...
    const sql = `
      INSERT INTO CatalogProduct (shop, sku, title, categoryMain, categorySegments, vendor, vendorNorm, stock, franchises, franchiseRefs, resolvedFranchise, resolvedFranchiseLayer, resolvedLine, netPrice, grossPrice, imageUrl, barcode, weightKg, titleSource, dimensions, hsCode, originalTitle, cleanTitle, resolvedFormat, resolvedManufacturerLine, indexedAt)
      ...
      ON CONFLICT(shop, sku) DO UPDATE SET
        ...
        franchiseRefs = excluded.franchiseRefs,
        resolvedFranchise = excluded.resolvedFranchise,
        resolvedFranchiseLayer = excluded.resolvedFranchiseLayer,
        resolvedLine = excluded.resolvedLine,
        ...
        resolvedFormat = excluded.resolvedFormat,
        resolvedManufacturerLine = excluded.resolvedManufacturerLine,
