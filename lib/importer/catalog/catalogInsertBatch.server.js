...
export async function insertCatalogProductBatch(shop, chunk, options = {}) {
  ...
  // 14 parâmetros por produto * 50 produtos = 700 (< limite de 999 do SQLite)
  const CHUNK = 50;
  for (let i = 0; i < unique.length; i += CHUNK) {
    ...
    const sql = `
      INSERT INTO CatalogProduct (shop, sku, title, categoryMain, categorySegments, vendor, vendorNorm, stock, franchises, netPrice, grossPrice, imageUrl, barcode, weightKg, indexedAt)
