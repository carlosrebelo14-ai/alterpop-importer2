...
export async function insertCatalogProductBatch(shop, chunk, options = {}) {
  ...
  // 20 parâmetros por produto * 45 produtos = 900 (< limite de 999 do SQLite)
  const CHUNK = 45;
  for (let i = 0; i < unique.length; i += CHUNK) {
    ...
    const sql = `
      INSERT INTO CatalogProduct (shop, sku, title, categoryMain, categorySegments, vendor, vendorNorm, stock, franchises, franchiseRefs, resolvedFranchise, resolvedFranchiseLayer, netPrice, grossPrice, imageUrl, barcode, weightKg, titleSource, dimensions, hsCode, indexedAt)
