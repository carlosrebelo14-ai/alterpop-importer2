...
import { resolveCharacters } from "./characterResolver.server.js";
...
export function toLiteProduct(r) {
  ...
  // o sinal costuma estar no próprio prefixo que a limpeza vai remover. R2 (17/09):
  // vendor entra para o fallback de manufacturer_line quando o título não dá formato.
  const resolvedFormat = extractProductFormat({ title, vendor: r.vendor || "" });
  ...
  // B7 (ADENDA 3, 17/09/2026) — resolvedCharacters recalculado no ingest, como
  // resolvedFormat/cleanTitle acima. Sem titleOverride aqui: o feed nunca traz um (é
  // campo do curador, só existe depois na linha Prisma); character-backfill.js e o
  // publish-payload continuam a preferir titleOverride quando ele já existir.
  const resolvedCharactersList = resolveCharacters({ originalTitle: title, resolvedFranchise: resolved.franchise });
  ...
  return {
    ...
    resolvedCharacters: resolvedCharactersList ? JSON.stringify(resolvedCharactersList) : null,
    categoryMain,
