  ...
  if (opts.sortBy === "margin") {
    ...
    const dataRows = await safePrisma("catalog.query.page", () =>
      prisma.$queryRawUnsafe(
        `SELECT p.shop, p.sku, p.title, p.categoryMain, p.categorySegments, p.vendor, p.stock, p.franchises, p.netPrice, p.grossPrice, p.imageUrl, p.barcode, p.titleSource, p.dimensions, p.hsCode
...
async function queryProductsWithFilterTags(shop, opts) {
  ...
  const dataRows = await safePrisma("catalog.query.page", () =>
    prisma.$queryRawUnsafe(
      `SELECT DISTINCT p.shop, p.sku, p.title, p.categoryMain, p.categorySegments, p.vendor, p.stock, p.franchises, p.netPrice, p.grossPrice, p.imageUrl, p.barcode, p.titleSource
