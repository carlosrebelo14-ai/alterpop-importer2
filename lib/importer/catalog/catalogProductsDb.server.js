  ...
  await safePrisma("catalogProduct.createMany", () =>
    prisma.catalogProduct.createMany({
      data: chunk.map((p) => ({
        ...
        franchiseRefs: JSON.stringify(p.franchiseRefs || []),
        resolvedFranchise: p.resolvedFranchise ?? null,
        resolvedFranchiseLayer: p.resolvedFranchiseLayer ?? null,
        resolvedLine: p.resolvedLine ?? null,
        ...
        cleanTitle: p.cleanTitle ?? null,
        resolvedFormat: p.resolvedFormat ?? null,
        resolvedManufacturerLine: p.resolvedManufacturerLine ?? null,
        resolvedCharacters: p.resolvedCharacters ?? null,
      })),
    })
  );
...
async function queryProductsPrismaPaginated(shop, opts) {
  ...
  if (opts.sortBy === "margin" || opts.sortBy === "confidence" || minConfidence != null) {
    ...
    // getSqlOrderBy já cobre todos os sortBy (netPrice/stock/title/indexedAt/margin/
    // confidence/default) — usar sempre, sem truncar aos dois casos que originalmente
    // forçavam este ramo SQL. Bug encontrado em code review 2026-08-13: quando
    // minConfidence estava definido com sortBy="netPrice" (ou stock/title), a
    // ordenação colapsava silenciosamente para p.sku ASC.
    const orderBySql = getSqlOrderBy(opts.sortBy, opts.sortDir);
