...
export async function applySmartRulesToBatch(batch) {
  const { upsertCurationQueueFromRecord } = await import(
  ...
  for (const lite of batch) {
    await upsertCurationQueueFromRecord({
      ...
      title: lite.title,
      vendor: lite.vendor,
      categoryMain: lite.categoryMain,
      category: lite.categoryMain,
      categorySegments: lite.categorySegments,
      netPrice: lite.netPrice,
      grossPrice: lite.grossPrice,
      availableQuantity: lite.stock,
      franchises: lite.franchises,
    });
  }
}
...
function appendSkuFilterWhere(where, params, skuInclude, skuExclude) {
  ...
  if (skuExclude && Array.isArray(skuExclude) && skuExclude.length > 0) {
    where.push(`p.sku NOT IN (${skuExclude.map(() => "?").join(",")})`);
    params.push(...skuExclude);
  }
}
...
function buildPrismaWhere(shop, { brand, search, minPrice, maxPrice, inStockOnly, skuInclude, skuExclude }) {
  ...
  if (skuExclude && Array.isArray(skuExclude) && skuExclude.length > 0) {
    where.sku = { ...where.sku, notIn: skuExclude };
  }
  return where;
}
...
async function queryProductsPrismaPaginated(shop, opts) {
  ...
  if (opts.sortBy === "margin") {
    ...
    if (search) { sqlWhere.push("(p.sku LIKE ? OR p.title LIKE ?)"); params.push(`%${search}%`, `%${search}%`); }
    ...
    const dir = String(opts.sortDir).toLowerCase() === "desc" ? "DESC" : "ASC";
    const dataRows = await safePrisma("catalog.query.page", () =>
      prisma.$queryRawUnsafe(
        `SELECT p.shop, p.sku, p.title, p.categoryMain, p.categorySegments, p.vendor, p.stock, p.franchises, p.netPrice, p.grossPrice, p.imageUrl, p.barcode
         FROM CatalogProduct p
         WHERE ${whereSql}
         ORDER BY (CASE WHEN p.grossPrice IS NOT NULL AND p.grossPrice > 0 AND p.netPrice IS NOT NULL THEN ((p.grossPrice - p.netPrice) / p.grossPrice) ELSE -999 END) ${dir}, p.sku ASC
         LIMIT ? OFFSET ?`,
        ...params,
        limit,
        skip
  ...
  const where = buildPrismaWhere(shop, { brand, search, minPrice, maxPrice, inStockOnly, skuInclude: opts.skuInclude, skuExclude: opts.skuExclude });
...
async function queryProductsWithFilterTags(shop, opts) {
  ...
  if (search) {
    where.push("(p.sku LIKE ? OR p.title LIKE ?)");
  ...
  const dataRows = await safePrisma("catalog.query.page", () =>
    prisma.$queryRawUnsafe(
      `SELECT DISTINCT p.shop, p.sku, p.title, p.categoryMain, p.categorySegments, p.vendor, p.stock, p.franchises, p.netPrice, p.grossPrice, p.imageUrl, p.barcode
