...
/**
 * @param {Object} input
 * @param {Map<string, { universo: string, productSkus: string[], productIds: string[] }>} input.liveCounts
 *   handle -> universo + SKUs/IDs ACTIVE que resolveram para esse handle
 * @param {Map<string, { id: string, status: 'ACTIVE'|'DRAFT', productIds: string[] }>} input.existing
 *   handle -> metaobject já existente na loja (ausente = nunca criado); productIds é o
 *   valor atual do campo "products", usado para detetar update sem mudança real.
...
export function planCharacterMetaobjects({ liveCounts, existing }) {
  ...
  for (const handle of handles) {
    ...
    if (count >= CHARACTER_THRESHOLD) {
      // Auditoria 2026-09-22 — um Character já ACTIVE e acima do limiar reemitia
      // "update" TODOS os ciclos, mesmo sem nenhuma mudança real de composição, por
      // não comparar contra o `products` já gravado no metaobject. Isso acumulava
      // "changing" sem fim (sempre >= 1 por Character estável), e assim que uma loja
      // passava a ter mais Characters estáveis do que MAX_CHARACTER_CHANGES_PER_CYCLE
      // (characterPagesReconcileCycle.server.js), o travão disparava red PARA SEMPRE
      // — nem sequer Characters genuinamente novos abaixo do limiar chegavam a
      // aplicar. Só emite "update" quando o conjunto de produtos realmente mudou.
      if (ex && ex.status === "ACTIVE") {
        const liveIds = new Set(live.productIds || []);
        const exIds = new Set(ex.productIds || []);
        const unchanged = liveIds.size === exIds.size && [...liveIds].every((id) => exIds.has(id));
        if (unchanged) {
          actions.push({ handle, nome, universo, action: "skip", count });
          continue;
        }
      }
