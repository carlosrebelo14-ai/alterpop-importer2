...
/** Ver SKU_FILTER_CHUNK_SIZE em catalogProductsDb.server.js — mesmo limite. */
const DEDUP_DELETE_CHUNK_SIZE = 500;
...
export async function deduplicateCatalog(shop) {
  ...
  // Fix (code review 2026-08-13): chunk de 500 (mesmo limite usado noutros pontos do
  // código — ver SKU_FILTER_CHUNK_SIZE em catalogProductsDb.server.js) para não
  // arriscar o limite de parâmetros do SQLite; `rethrow: true` para que uma falha
  // real na transação PARE aqui e propague, em vez de deixar `summary.removed`
  // (calculado antes, a partir de removedSkus.length) a reportar sucesso quando o
  // delete pode não ter acontecido de todo.
  for (const chunk of chunkArray(removedSkus, DEDUP_DELETE_CHUNK_SIZE)) {
    await safePrisma(
      "dedup.delete",
      () =>
        prisma.$transaction([
