 ...
 *
 *  AVERBAMENTO À DECISÃO 17 (2026-09-14) — a Decisão 17 fica REVERTIDA na parte da
 *  remoção e MANTIDA na parte da extração. Em 11/09 tirou-se este vocabulário da lista
 *  por ser "informação de formato genuína, não ruído", e estava certo — mas foi decidido
 *  com o título ESCONDIDO na PDP. Desde 13/09 o título é visível por baixo do slot
 *  Character, e 4 331 produtos mostravam "POP figure …" no título enquanto
 *  `alterpop.format` mostrava "Figure" no seu próprio slot: o cliente lia a mesma
 *  informação duas vezes.
 *
 *  A extração para `alterpop.format` continua, e é ela que torna a remoção segura — ver
 *  a guarda em titleCleaner.server.js: o prefixo só sai quando o formato ficou guardado
 *  noutro sítio. Lista reposta pelo censo da Tarefa 31.
 *
 *  Ordenados por comprimento desc — o mais específico primeiro ("Blister 4 figures Bitty
 *  POP" antes de "POP figure"). */
...
/** Os dois que NÃO são formato: "Assorted" (variante aleatória de uma caixa) e "Latino"
 *  (locale do fornecedor). A Decisão 17 classificou-os como "ruído sem informação
 *  nenhuma" e é por isso que saem sempre, sem depender de `resolvedFormat` — não há
 *  informação nenhuma para mudar de sítio. A guarda do averbamento aplica-se só ao
 *  vocabulário de formato reposto. */
/** "Rides" — B18 (briefing backend, 17/09/2026), achado na auditoria do live: título
 *  "Rides Star Wars Luke Skywalker in T-47 Airspeeder Exclusive" (SKU 889698744041).
 *  Resto de reestruturação de frase do fornecedor ("X Rides Y in Z" → o "Rides" sobra
 *  na frente quando o título é reordenado) — não carrega informação de formato nenhuma,
 *  mesmo espírito de Assorted/Latino. Corrigido pela regra (aqui), não no produto. */
export const NOISE_PREFIXES = ["Assorted", "Latino", "Rides"].sort((a, b) => b.length - a.length);
 ...
 *  lógica de match. */
export function matchFormatPrefix(title, prefixes = FORMAT_PREFIXES) {
...
export function stripFormatPrefix(title, prefixes = FORMAT_PREFIXES) {
  const t = String(title || "").trim();
  const prefix = matchFormatPrefix(t, prefixes);
  if (!prefix) return t;
