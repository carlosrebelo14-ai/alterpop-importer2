...
// ── Tarefa 14 — camada 3 "chapéu" (Disney, Marvel) ────────────────────────────
// NÃO ligada a resolveFranchise() nem ao publisher. Só para o dry-run
// (scripts/catalog/franchise-umbrella-dryrun.js) até haver aprovação explícita —
// "Não escrever nada sem dry-run aprovado" (briefing). Precedência: testada só
// depois de camadas 1–2 (+ Lines) não terem resolvido nada.

const UMBRELLA_UNIVERSES = [
  { name: "Marvel", handle: "marvel", refs: ["MARVEL"] },
  { name: "Disney", handle: "disney", refs: ["DISNEY"] },
];

const UMBRELLA_REF_INDEX = (() => {
  const map = new Map();
  for (const u of UMBRELLA_UNIVERSES) {
    for (const ref of u.refs) map.set(normRef(ref), u);
  }
  return map;
})();

const UMBRELLA_TITLE_CANDIDATES = UMBRELLA_UNIVERSES.map((u) => ({
  universe: u,
  needle: ` ${normText(u.name)} `,
}));

/**
 * Camada 3 chapéu: só chamada para produtos que resolveFranchise() já devolveu vazio.
 * Devolve { franchise, handle, matchedOn, umbrella: true } ou null.
 * @param {{ franchiseRefs?: string[], title?: string }} product
 */
export function resolveUmbrella(product) {
  const refs = Array.isArray(product?.franchiseRefs) ? product.franchiseRefs : [];
  for (const raw of refs) {
    const u = UMBRELLA_REF_INDEX.get(normRef(raw));
    if (u) return { franchise: nfc(u.name), handle: u.handle, matchedOn: raw, umbrella: true };
  }
  const haystack = ` ${normText(stripFormatPrefix(product?.title || ""))} `;
  for (const cand of UMBRELLA_TITLE_CANDIDATES) {
    if (haystack.includes(cand.needle)) {
      return { franchise: nfc(cand.universe.name), handle: cand.universe.handle, matchedOn: cand.universe.name, umbrella: true };
    }
  }
  return null;
}
...
/**
 * Tarefa 14b (Decisão 9) — resolveFranchise() + resolveUmbrella() compostos, para a
 * prova de não regressão. NÃO É a função usada pela indexação nem pelo publisher —
 * essas continuam a chamar resolveFranchise() sem chapéu. Esta função só existe para
 * `franchise-umbrella-nonregression.js` medir, sobre o catálogo real, se ligar o
 * chapéu deixaria intactos os produtos já resolvidos por camada 1/2/Line (Condição
 * "Não regressão" da Decisão 9).
 *
 * Precedência: camadas 1-2 (+ Lines) primeiro, exatamente como resolveFranchise()
 * hoje; o chapéu só é tentado quando isso devolve vazio. Por construção, um produto
 * que passe a resolver por um universo próprio (ex.: Deadpool → X-Men, Tarefa 13b)
 * nunca mais chega ao chapéu — não precisa de nenhuma migração manual para "sair" do
 * chapéu quando a tabela ganha uma entrada própria (Condição "Reversibilidade").
 *
 * @param {{ franchiseRefs?: string[], title?: string }} product
 * @param {object} [opts]
 * @returns {ReturnType<typeof resolveFranchise> & { isUmbrella: boolean }}
 */
export function resolveFranchiseWithUmbrella(product, opts = {}) {
  const base = resolveFranchise(product, opts);
  if (base.franchise) return { ...base, isUmbrella: false };

  const hit = resolveUmbrella(product);
  if (!hit) return { ...base, isUmbrella: false };
