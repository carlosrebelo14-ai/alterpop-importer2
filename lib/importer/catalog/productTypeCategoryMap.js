...
export const PRODUCT_TYPE_SEGMENT_TO_CATEGORY = {
  ...
  BISUTERIA: "Jewellery",
  PENDIENTES: "Earrings",
  // RELOJES was incorrectly mapped to Apparel (clothing), depois para o nó genérico
  // "Clocks" (hg-3-17). O catálogo só tem relógios de parede/decorativos, não de
  // pulso — hg-3-17-4 ("Relógios de parede") é o nó específico correto.
  RELOJES: "WallClock",
  ...
  // Variantes em inglês — produtos com productTypePath já traduzido pelo fornecedor
  // (titleSource "supplier") trazem o segmento em inglês em vez de espanhol. Sem
  // estas chaves, os mesmos tipos de produto que já resolvem categoria via chave
  // espanhola (FIGURAS, PELUCHES, TAZAS, JUGUETES) ficavam sem categoria só por
  // virem em inglês. Categoria espelhada 1:1 da chave espanhola equivalente —
  // nenhuma categoria nova, só sincronização de idioma (achado do levantamento
  // dos 53% sem categoria, 2026-08-12).
...
/**
 * \u00cdndice normalizado de PRODUCT_TYPE_SEGMENT_TO_CATEGORY \u2014 as chaves do mapa acima
 * s\u00e3o escritas \u00e0 m\u00e3o com pontua\u00e7\u00e3o leg\u00edvel (ex: "CAMISETAS M/C"), mas normalizeSegment
 * agora remove pontua\u00e7\u00e3o. Sem este \u00edndice, normalizeSegment("CAMISETAS M/C") deixaria
 * de bater certo com a chave literal do objeto. Constru\u00eddo uma vez, no load do m\u00f3dulo.
 */
const NORMALIZED_SEGMENT_TO_CATEGORY = Object.fromEntries(
  Object.entries(PRODUCT_TYPE_SEGMENT_TO_CATEGORY).map(([key, value]) => [
...
export function categoryKeyFromProductTypePath(productTypePath) {
  ...
  for (const idx of [1, 0]) {
    const seg = parts[idx];
    if (!seg) continue;
    const key = NORMALIZED_SEGMENT_TO_CATEGORY[normalizeSegment(seg)];
    if (key) return key;
  }
  return undefined;
}
