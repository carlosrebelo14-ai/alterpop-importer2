...
export function catalogRowToEvaluateRecord(row) {
  ...
  // Auditoria 2026-09-22 — faltava aqui (bug encontrado, `record.franchises` sempre
  // undefined). evaluateStructuredCatalogFilter.server.js (detectEliteUniverse,
  // hasPremiumFranchiseTag) lê `record.franchises` para reconhecer um produto
  // fast-tracked por tag de franquia no import original (structured_franchise_tag /
  // elite_short_circuit:franchise:...). Sem isto, uma reavaliação de purga nunca via
  // esse sinal e podia apagar permanentemente um produto elite/VIP legítimo por
  // parecer "não premium" — não porque o produto o fosse, mas porque o record
  // reconstruído tinha perdido o dado.
  let franchises = [];
  try {
