...
// Histórico (2026-08-03): indexações de catálogos grandes (~30k produtos) sofriam
// um stall a partir dos ~25k linhas. Isolado com global.gc() forçado: o inMemoryQueue
// de curadoria crescia sem limite (~700 bytes/produto, nunca esvaziado até ao fim do
// stream), gerando pressão de GC severa. RESOLVIDO com o flush incremental abaixo
// (CURATION_FLUSH_INTERVAL) — o heap mantém-se agora estável (40-90MB) durante toda
// a indexação, confirmado em produção.
//
// Continua a existir, na MESMA zona (~25k+), um abrandamento residual de ~15-20x no
// rate de processamento (ex.: 500/s → 25-35/s) que NÃO está associado a crescimento
// de heap — logo não é o mesmo problema, e ainda não foi diagnosticado. Suspeita
// principal: custo de inserção/índices SQLite a aumentar com o tamanho da tabela.
// Não é urgente: a app continua funcional (~6min para indexar ~30k produtos, dentro
// do histórico). Se for preciso investigar, reintroduzir timing local (insert vs
// regras de curadoria vs outros) só na janela 20k-30k.
...
export async function syncCatalogWithProgress(shop, settings = {}, hooks = {}) {
  ...
  if (clearFirst) {
    ...
    // rethrow:true — um clear que falha em silêncio e é seguido de inserts sobre o
    // catálogo antigo intacto corrompe os dados sem deixar rasto (mesma classe do
    // incidente CatalogSkuTracking de 24/09). O worker fork tem catch próprio
    // (catalog-index-worker.mjs) que reporta a falha com mensagem real em vez de
    // continuar às cegas.
    await safePrisma(
      "catalog.clear",
      () =>
