...
/**
 * @param {{ insertMs: number, rulesMs: number, otherMs: number, batchCount: number, mapMs: number, normalizeMs: number, excludeMs: number, facetMs: number }} acc
 * @param {number} scanned
 */
function logBatchTiming(acc, scanned) {
  console.log(
    `[indexing:timing] até linha ${scanned} (${acc.batchCount} batches) — insert=${acc.insertMs}ms rules=${acc.rulesMs}ms map=${acc.mapMs}ms normalize=${acc.normalizeMs}ms exclude=${acc.excludeMs}ms facet=${acc.facetMs}ms outros=${acc.otherMs}ms`
  );
}
// FIM TEMP
