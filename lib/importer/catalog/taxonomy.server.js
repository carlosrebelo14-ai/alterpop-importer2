...
/** Cached matcher map — built once, reused for all products. */
let _cachedMatcherMap = null;
...
export function computeTaxonomyFilterTagIds(product) {
  if (!_cachedMatcherMap) {
    _cachedMatcherMap = buildTaxonomyMatcherMap();
  }
  const ids = [];
  for (const [id, fn] of _cachedMatcherMap) {
