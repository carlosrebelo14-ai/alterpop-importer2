...
export function cleanProductTitleWithTrace(product = {}, opts = {}) {
  ...
  //
  // BUG (auditoria live, 17/09/2026, item 5) — a versão anterior escolhia UMA lista por
  // produto (NOISE_PREFIXES OU FORMAT_PREFIXES, nunca as duas) com base em
  // `resolvedFormat`. Para "POP figure Rides Star Wars Luke Skywalker in T-47
  // Airspeeder Exclusive" (resolvedFormat="Figure", achado pelo próprio "POP figure"),
  // isso escolhia só FORMAT_PREFIXES: a 1.ª passagem tirava "POP figure" (está nas
  // duas), a 2.ª já não encontrava "Rides" (só está em NOISE_PREFIXES) e parava —
  // ficava por limpar. NOISE_PREFIXES não carrega informação nenhuma (comentário acima
  // de NOISE_PREFIXES em franchiseResolver.server.js: "sai sempre, sem depender de
  // resolvedFormat"), por isso tenta-se em TODAS as passagens; FORMAT_PREFIXES só
  // quando a guarda permite.
  let passagens = 0;
  ...
  if (opts.manterPrefixo) {
    ...
    while (passagens < MAX_PASSAGENS_PREFIXO) {
      let candidato = stripFormatPrefix(t, NOISE_PREFIXES);
      if (candidato === t && product.resolvedFormat) {
        candidato = stripFormatPrefix(t);
      }
      if (candidato === t) {
