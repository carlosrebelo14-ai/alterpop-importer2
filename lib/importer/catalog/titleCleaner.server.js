...
export function cleanProductTitleWithTrace(product = {}) {
  ...
  const semPrefixo = listaAplicavel ? stripFormatPrefix(t, listaAplicavel) : stripFormatPrefix(t);
  if (semPrefixo !== t) {
    firedRules.push("prefixo-formato");
    t = semPrefixo;
  } else if (!product.resolvedFormat && stripFormatPrefix(t) !== t) {
    // Tinha prefixo de FORMATO mas não tem onde o guardar — fica como está, e conta-se.
    firedRules.push("prefixo-formato-travado");
  }
  ...
  const aliases = [...new Set([...franchiseAliases, ...lineAliases])];
