...
/** Travão 2 — o que pode sobrar depois de uma passagem. Nunca vazio. Nunca a string
 *  INTEIRA igual a um alias da franquia/line resolvida (briefing 15/09/2026 · B0 —
 *  compara a string inteira contra aliases multi-palavra, não só a última palavra
 *  isolada: "Harry Potter" sobrando sozinho colapsa o título para o nome da franquia, os
 *  19 casos de A). Uma palavra isolada continua a só sobrar se for ela própria um alias
 *  conhecido ("Batman" sim, "figure" não — lição da Tarefa 19/20). */
function sobraSuficiente(texto, aliasesNorm) {
  const t = String(texto || "").trim();
  if (!t) return false;
  // A string inteira (1 ou mais palavras) não pode ser, ela própria, um alias
  // conhecido — cobre tanto o caso de uma palavra ("Transformers") como o de um nome
  // de franquia/line com várias ("Harry Potter", "Sonic The Hedgehog").
  if (aliasesNorm.has(normalizeForMatch(t))) return false;
