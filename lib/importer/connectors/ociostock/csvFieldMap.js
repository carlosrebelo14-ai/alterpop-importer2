...
export function mapCsvRow(row, opts = {}) {
  ...
  const hsCodeField = opts.hsCodeField || "hs_intrastat_code";
  ...
  const { franchiseRefs: familyRefs, categoryTokens: familyCategoryTokens } = parseFamilySignals(
    stripQuotes(row[familiesXmlField]),
    { categoryRaw, productTypeXml, title: titleRaw, description: descriptionRaw }
  );
  // `franchises` mantém-se exatamente como antes (refs + tokens de categoria, achatados) —
  // continua a alimentar facetas, filtros de catálogo, taxonomia e curatorChat.
  const franchises = [...new Set([...familyRefs, ...familyCategoryTokens])];
  // `franchiseRefs` é novo: só os ref="..." do fornecedor, sem tokens de categoria e sem
  // passar por tradução. Chave da camada 1 do franchiseResolver (ver docs/normalizacao-franquias.md).
  const franchiseRefs = [...new Set(familyRefs)];
  ...
  const record = {
    ...
    /** pauta aduaneira normalizada (6 ou 8 dígitos); null nos ~9,3% sem dado válido */
    hsCode: parseHsCode(stripQuotes(row[hsCodeField])),
    ...
    productTypePath,
    franchises,
    /** Só os ref="..." do fornecedor (vocabulário controlado, não traduzido). Camada 1
     *  do franchiseResolver. NÃO passa por translate.js — ver invariante no plano. */
    franchiseRefs,
