 ...
 * - título e descrição (texto livre — ex.: "One Piece" em espanhol/inglês)
 *
 ...
 * @param {string} [context.title] - nombre
 * @param {string} [context.description] - descripcion
 * @returns {string[]}
 */
export function parseFranchiseRefs(xmlFamilies, context = {}) {
  ...
  const { categoryRaw, productTypeXml, title, description } = context;
  ...
  if (title) addFranchiseTokens(title, bucket);
  if (description) addFranchiseTokens(description, bucket);
