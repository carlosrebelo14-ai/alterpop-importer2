...
/**
 * Separa os dois tipos de sinal que o feed OcioStock mistura debaixo de "família":
 *
 * - `franchiseRefs` — SÓ os atributos `ref="..."` de `xml_info_familias`. Vocabulário
 *   controlado do fornecedor (`BLACKCLOVER`, `STAR_WARS_EP`, `FUNKO`), estável e não
 *   traduzido. É a chave de mapeamento da camada 1 do franchiseResolver — não pode
 *   passar por tradução nem ser diluída com tokens de categoria livre.
 * - `categoryTokens` — todo o resto: CDATA e texto de `<category>`, `categoria_principal`
 *   (segmentos pipe) e `product_type_path`. Árvore de categorias do fornecedor, muitas
 *   vezes em espanhol e com ruído (`POP CULTURE COLLECTIBLES`, `ANIME / MANGA`).
 *
 * `parseFranchiseRefs()` mantém-se como wrapper que volta a juntar os dois, byte a byte
 * com o comportamento histórico, para os ~20 consumidores de `record.franchises`.
 *
 ...
 * @returns {{ franchiseRefs: string[], categoryTokens: string[] }}
 */
export function parseFamilySignals(xmlFamilies, context = {}) {
  const refs = new Set();
  ...
  if (xmlFamilies) {
    ...
    while ((m = refRe.exec(xmlFamilies)) !== null) {
      if (m[1]?.trim()) refs.add(m[1].trim());
    }

    for (const token of extractXmlTextTokens(xmlFamilies)) {
      addFranchiseTokens(token, categoryTokens);
    }
  }
  ...
  if (categoryRaw) {
    addFranchiseTokens(categoryRaw, categoryTokens);
  }

  if (productTypeXml) {
    const { path, segments } = parseProductTypePath(productTypeXml);
    if (path) addFranchiseTokens(path, categoryTokens);
    for (const seg of segments) {
      if (seg) categoryTokens.add(seg);
    }
  ...
  return { franchiseRefs: [...refs], categoryTokens: [...categoryTokens] };
}

/**
 * Sinais de franchise do feed OcioStock, achatados num único array (refs primeiro,
 * depois tokens de categoria), com a 1.ª ocorrência a vencer em caso de repetição.
 *
 * @deprecated para lógica nova de franquia usa {@link parseFamilySignals} e trata os
 * `franchiseRefs` isoladamente. Este wrapper existe só para não partir os consumidores
 * históricos de `record.franchises` (facetas, filtros, taxonomia, curatorChat).
 *
 * @param {string} xmlFamilies - coluna xml_info_familias
 * @param {object} [context]
 * @param {string} [context.categoryRaw] - categoria_principal
...
export function parseFranchiseRefs(xmlFamilies, context = {}) {
  ...
  return [...new Set([...franchiseRefs, ...categoryTokens])];
}
