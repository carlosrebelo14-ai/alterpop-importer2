...
function getDynamicRuleSets() {
  ...
  return {
    ...
    // blockedTerms NÃO entra aqui — entrava antes (bug: aprovava produtos que devia
    // rejeitar, porque detectEliteUniverse corre antes do passo que rejeita por
    // junkCategoryContains). blockedTerms só deve alimentar junkCategoryContains.
    eliteKeywords: [...new Set([...market.vipBrands, ...market.vipLicences])],
...
function loadTaxonomyMap() {
  ...
  try {
    ...
    console.warn(`⚠️ taxonomy-map.json erro: ${err.message}`);
    console.warn("Fallback vazio — categorização desativada");
    return { sections: [], productTypes: [] };
  }
}
