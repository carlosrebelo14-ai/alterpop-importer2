...
export class ProductImporter extends BaseImporter {
  ...
  async upsertOne(record) {
    ...
    // Briefing 16/09/2026 — a curadoria por Gemini foi removida daqui. A aprovação do
    // Carlos no painel é a decisão final; nada a jusante recusa nem aprova por conta
    // própria. Regra explícita: se algo precisar de travar um produto depois da
    // aprovação, falha com erro visível, nunca com um estado silencioso.
  ...
  async updateProduct(record, variant, productType) {
    // syncLocked vem de resolveVariantBySku() (mesma query que já resolveu a
    // variante) — não faz uma chamada GraphQL extra por produto.
