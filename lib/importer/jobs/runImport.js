...
import { syncFranchiseCatalog } from "../shopify/franchiseCatalogSync.server.js";
...
export async function runImport(options = {}) {
  ...
  const inventoryImporter = syncInventory
    ? new InventoryImporter(job, activeClient, settings)
    : null;
  ...
  // Recalcula o catálogo de franquias com stock para a grelha do tema (ver
  // franchiseCatalogSync.server.js). Nunca deve fazer o import falhar — a função já
  // engole os seus próprios erros e devolve { ok:false }, aqui só regista.
  if (client) {
    const franchiseResult = await syncFranchiseCatalog(client, options.session?.shop || "");
    if (!franchiseResult.ok) {
      job.recordFailed({
        sku: "(franchise_catalog)",
        type: "franchise_catalog_sync",
        reason: franchiseResult.error,
      });
    }
  }
