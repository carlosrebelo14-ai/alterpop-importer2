...
export async function fetchExistingCharacters(client) {
  ...
  for (;;) {
    ...
    for (const n of conn?.nodes || []) {
      // productIds — auditoria 2026-09-22: sem isto, planCharacterMetaobjects não
      // tinha como saber se um Character já ACTIVE realmente mudou de composição, e
      // reemitia "update" para todos, sempre — ver comentário em characterPages.server.js.
      let productIds = [];
      try {
        productIds = JSON.parse(n.productsField?.value || "[]");
      } catch {
        productIds = [];
      }
      existing.set(n.handle, {
        id: n.id,
