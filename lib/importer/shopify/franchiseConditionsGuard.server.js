/**
 * BRIEFING BACKEND · Tarefa 26 — guarda permanente. Tarefa 30 (Decisão 15) — guarda
 * tri-direcional.
 *
 ...
 *
 * `computeFranchiseConditionsDiff(client)` — pura leitura, devolve as 3 direções de
 * divergência:
 *   1. universo/line ATIVO na tabela sem coleção na loja (ou condição/templateSuffix
 *      errados)
 *   2. coleção Universe/Line na loja sem entrada na tabela ("stray")
 *   3. universo DORMENTE na tabela COM coleção na loja — inconsistência (Tarefa 30):
 *      ou ficou stale de quando era ativo, ou o baseline voltou a cruzar o limiar sem
 *      a tabela ser atualizada
 * mais os problemas de NFC da lista versionada. `assertFranchiseConditionsAligned(client)`
...
export async function computeFranchiseConditionsDiff(client) {
  ...
  for (const want of FRANCHISE_CONDITIONS) {
    ...
    // Tarefa 30 (Decisão 15) — 3.ª direção da guarda: um universo DORMENTE não devia
    // ter coleção nenhuma na loja (se tem, ou ficou stale de quando era ativo, ou o
    // baseline voltou a cruzar o limiar sem a tabela ser atualizada — as duas pedem
    // decisão humana, não publish silencioso).
    if (want.dormant) {
      diffs.push({ handle: want.handle, kind: want.kind, problem: "dormente mas tem coleção na loja — inconsistência" });
      continue;
    }
