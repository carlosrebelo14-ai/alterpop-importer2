 ...
 *
 * `single_line_text_field`, NÃO lista — ao contrário de franchise/line/format, que são
 * lista para o caso raro de crossover, o extrator só devolve UM valor (o mais
 * específico vence quando há sobreposição). A definição original saiu como
 * list.single_line_text_field por hábito de copiar as outras três, sem essa
 * justificação — apagada e recriada como escalar 16/09/2026, com 0 valores gravados
 * (verificado por query antes de apagar, sem migração de dados necessária).
 *
 * `smartCollectionCondition` ligado — single_line_text_field é o tipo que a doc da
 * Shopify documenta como suportado para smart collections (ao contrário do list.*, que
 * as outras três só confirmaram empiricamente).
 *
...
export const ALTERPOP_MANUFACTURER_LINE_DEFINITION = {
  ...
  description:
    "Linha de produto dentro do fabricante (ex.: Ichibansho dentro de Banpresto), atribuída pelo manufacturerLineExtractor a partir do título + vendor. Um valor por produto. Não confundir com alterpop.line.",
  ownerType: "PRODUCT",
...
/** GID da definição alterpop.manufacturer_line em jyr17t-wr.myshopify.com (recriada
 *  16/09/2026 como single_line_text_field — a original, list.single_line_text_field,
 *  foi apagada com 0 valores gravados). Reconfirmar por query se a loja for recriada. */
export const ALTERPOP_MANUFACTURER_LINE_DEFINITION_GID =
  "gid://shopify/MetafieldDefinition/1531472249162";
