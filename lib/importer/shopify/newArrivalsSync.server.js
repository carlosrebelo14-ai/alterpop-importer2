 ...
 * @returns {Promise<{ ok: boolean, windowDays?: number, tagged?: number, untagged?: number, scanned?: number, truncated?: boolean, error?: string }>}
 */
...
/**
 * Auditoria 2026-09-22 — o cap de MAX_PAGES (5000 produtos) parava a leitura sem
 * log nem sinal no retorno; se `query` tivesse mais matches do que isso, o resto
 * ficava silenciosamente fora do tagsAdd/tagsRemove desta corrida, sem ninguém
 * saber que a leitura tinha sido cortada. Devolve `truncated` para o chamador
 * decidir/registar — ver syncNewArrivalsTag().
 */
...
export async function syncNewArrivalsTag(client, shop, opts = {}) {
  ...
  try {
    ...
    const inWindowResult = await pageAll(client, `published_status:published AND published_at:>'${cutoffISO}'`);
    const staleResult = await pageAll(client, `tag:'${NEW_ARRIVAL_TAG}' AND published_at:<'${cutoffISO}'`);
    const inWindow = inWindowResult.nodes;
    const stale = staleResult.nodes;
    const truncated = inWindowResult.truncated || staleResult.truncated;
    if (truncated) {
      console.warn(
        `[new-arrivals] ${shop}: leitura truncada no limite de ${MAX_PAGES} páginas — alguns produtos podem ter ficado fora desta corrida (o próximo ciclo auto-corrige o resto).`
      );
    }
