...
/**
 * @param {{ shop: string, accessToken: string }} session
 * @param {{ skipInit?: boolean, customTags?: string[] }} [opts]
 */
export async function runApprovedShopifySync(session, opts = {}) {
  ...
  const uniqueSkus = [...new Set(approved)];
...
export function startApprovedShopifySyncInBackground(session, opts = {}) {
  setImmediate(() => {
    runApprovedShopifySync(session, { skipInit: true, customTags: opts.customTags }).catch((err) => {
