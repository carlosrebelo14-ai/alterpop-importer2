...
import { FRANCHISE_UNIVERSES } from "../catalog/franchiseUniverses.js";
import { FRANCHISE_LINES } from "../catalog/franchiseLines.js";
import { resolveManufacturerTier } from "../catalog/manufacturerTierResolver.server.js";
...
/** universo/line ATIVO → handle da coleção Shopify (B15) — link direto, sem risco de
 *  404, porque só entram aqui universos com coleção garantida (ver universeCollections
 *  .server.js). Universo dormente ou Line sem par cai no fallback por tag em
 *  buildLicenceUniverseLinks. */
const UNIVERSE_HANDLE_BY_NAME = new Map(
  FRANCHISE_UNIVERSES.filter((u) => u.active).map((u) => [u.name, u.handle])
);
const LINE_HANDLE_BY_NAME = new Map(FRANCHISE_LINES.map((l) => [l.line, l.handle]));
...
/**
 * Frase de abertura por nível de fabricante (B15, briefing 16/09/2026) — antes era
 * idêntica em todos os produtos. Só cobre os 28 fabricantes com tier resolvido
 * (manufacturerTiers.js); sem tier, cai no texto genérico de sempre.
 * @type {Record<import('../catalog/manufacturerTiers.js').ManufacturerTier, (title: string, vendor: string) => string>}
 */
const TIER_INTRO_HTML = {
  ARTISAN: (title, vendor) =>
    `<p><strong>${escapeHtml(title)}</strong> is a hand-finished, museum-quality piece from <strong>${escapeHtml(vendor)}</strong>, one of the most exclusive names in premium collectibles. For serious collectors and connoisseurs.</p>`,
  SIGNATURE: (title, vendor) =>
    `<p><strong>${escapeHtml(title)}</strong> is a premium showpiece from <strong>${escapeHtml(vendor)}</strong>, renowned for exceptional detail and craftsmanship. For collectors who expect the very best.</p>`,
  COLLECTOR: (title, vendor) =>
    `<p><strong>${escapeHtml(title)}</strong> is a premium collectible from <strong>${escapeHtml(vendor)}</strong>, a name collectors trust for quality and detail. For fans and collectors alike.</p>`,
  CORE: (title, vendor) =>
    `<p><strong>${escapeHtml(title)}</strong> is an official licensed collectible from <strong>${escapeHtml(vendor)}</strong>. Perfect for fans, collectors, and pop culture enthusiasts.</p>`,
};

/**
 * Links seguros para License / Universe (B15) — lê o universo/line RESOLVIDO
 * (alterpop.franchise / alterpop.line), nunca os tokens em bruto do feed
 * (product.franchises podia trazer "Xoff", "Offers", "Relojes" — árvore de categorias
 * do fornecedor, não licença). Link direto à coleção quando o handle é conhecido
 * (universo ativo / Line com par); fallback por tag, mesmo padrão seguro do 978b4b7,
 * quando não é (universo dormente sem coleção ainda).
 * @param {{ resolvedFranchise?: string|null, resolvedLine?: string|null }} product
 */
function buildLicenceUniverseLinks(product) {
  const entries = [];
  const universeName = String(product.resolvedFranchise || "").trim();
  const lineName = String(product.resolvedLine || "").trim();
  if (universeName) entries.push({ label: universeName, handle: UNIVERSE_HANDLE_BY_NAME.get(universeName) });
  if (lineName) entries.push({ label: lineName, handle: LINE_HANDLE_BY_NAME.get(lineName) });

  return entries.map(({ label, handle }) => {
    const url = handle ? `/collections/${handle}` : `/collections/all?filter.p.tag=${encodeURIComponent(label)}`;
    return `<a href="${url}" style="color:#005bd3;text-decoration:underline;font-weight:500;">${escapeHtml(label)}</a>`;
  });
}
...
export function buildProductDescriptionHtml(product) {
  ...
  // Category (B15): lê o segmento curado do product_type_path (~85% cobertura, 40
  // entradas revistas manualmente) em vez de categoryMain — categoria_principal do
  // fornecedor resolve para buckets de marketing/estado interno ("Clearance", "Xoff",
  // "Offers"), nunca tipo de produto real. Ver productTypeCategoryMap.js. Sem match
  // curado, a linha fica de fora — nunca mostra o valor em bruto.
  const cleanCategory = categoryKeyFromSegments(parseJsonArray(product.categorySegments)) || null;

  const franchiseLinksHtml = buildLicenceUniverseLinks(product);

  // Engaging English commercial intro — varia por nível de fabricante (B15) quando
  // resolvido; senão mantém o texto genérico de sempre.
  const tier = vendor ? resolveManufacturerTier(vendor) : null;
  const intro = tier && TIER_INTRO_HTML[tier]
    ? TIER_INTRO_HTML[tier](title, vendor)
    : (() => {
        let html = `<p><strong>${escapeHtml(title)}</strong> is an official high-quality collectible item`;
        if (vendor) html += ` licensed by <strong>${escapeHtml(vendor)}</strong>`;
        html += `. Perfect for fans, collectors, and pop culture enthusiasts.</p>`;
        return html;
      })();
  ...
  // Category / Type — texto simples, sem link: cleanCategory vem do segmento curado
  // do product_type_path (categoryKeyFromSegments), uma fonte diferente do productType
  // guardado no produto Shopify (resolveShopifyProductType(categoryMain)). Um filtro
  // filter.p.product_type= sobre cleanCategory arriscava devolver zero resultados —
  // "seguro" mas inútil, o mesmo problema que o 978b4b7 corrigiu para License/Universe.
  if (cleanCategory) {
    specRows.push(`<tr>
      ...
      <td style="padding:10px 14px;border:1px solid #e9ecef;color:#495057;">${escapeHtml(cleanCategory)}</td>
    </tr>`);
  }

  // License / Universe (B15) — franchiseLinksHtml já vem pronto de
  // buildLicenceUniverseLinks: lê alterpop.franchise/alterpop.line resolvidos, nunca os
  // tokens em bruto do feed.
  if (franchiseLinksHtml.length > 0) {
    specRows.push(`<tr>
      ...
      <td style="padding:10px 14px;border:1px solid #e9ecef;">${franchiseLinksHtml.join(", ")}</td>
    </tr>`);
  }
...
/**
 * buildPublishPayload — B18 (briefing backend, 17/09/2026), item 1 da ordem.
 *
 * Os valores finais que dependem de resolução/tradução — título, franquia, line,
 * formato, manufacturer_line, preço e descrição — computados AQUI, uma vez só. O
 * publisher (mapCatalogProductToShopifyPayload, abaixo) chama esta função em vez de
 * recalcular os mesmos valores; prePublishChecks.server.js e a auditoria do live
 * (live-publish-audit.js) chamam-na directamente, exactamente como o publisher — nunca
 * uma segunda implementação que possa divergir em silêncio.
 *
 * "Pura" no sentido que interessa aqui: sem escrita nenhuma (não toca em Prisma nem
 * Shopify). Não é síncrona — a tradução do título é uma chamada de rede (só quando
 * titleSource !== "supplier" e não há título final já resolvido por outra via); dado o
 * mesmo `product`, sem side effects.
 *
 * @param {{
 *   sku: string,
 *   title: string,
 ...
 *   resolvedManufacturerLine?: string|null,
 *   resolvedCharacters?: string[]|null,
 *   netPrice?: number|null,
 * }} product
 */
export async function buildPublishPayload(product) {
  ...
  // Tarefa 10 (normalização de títulos, 2026-09-11) — baseTitle é o cleanTitle do
  // titleCleaner.server.js quando já foi calculado (corre na indexação, depois do
  // resolver); NULL nas linhas ainda não passadas pelo backfill, onde cai para
  // rawTitle e o comportamento fica exactamente como antes.
  const baseTitle = String(product.cleanTitle || rawTitle || "").trim();
  let translatedTitle = baseTitle;
  ...
  if (baseTitle && product.titleSource !== "supplier") {
    try {
      translatedTitle = await translateTitleWithApiFallback({ title: baseTitle, vendor, franchises });
    } catch {
      translatedTitle = baseTitle;
    }
  }
  // titleOverride (curador, manual) vence sempre que definido — nunca passa pela
  // tradução, é o texto final tal como o curador o escreveu.
  const titleOverride = String(product.titleOverride || "").trim();
  const title = (titleOverride || translatedTitle || rawTitle || `Product ${sku}`).replace(/[\r\n\t]/g, " ").trim();
  ...
  return {
    ...
    resolvedLine: product.resolvedLine ?? null,
    resolvedFormat: product.resolvedFormat ?? null,
    resolvedManufacturerLine: product.resolvedManufacturerLine ?? null,
    // B7 — resolvedCharacters já vem calculado (indexação/backfill, characterResolver.server.js
    // a partir de titleOverride/originalTitle + resolvedFranchise); aqui só passa por diante,
    // como os outros campos `resolved*` acima.
    resolvedCharacters: product.resolvedCharacters ?? null,
    retailPrice,
    descriptionHtml,
  };
}
...
export async function mapCatalogProductToShopifyPayload(product, opts = {}) {
  ...
  // B18 — título, franquia/line/formato/manufacturer_line, preço e descrição vêm todos
  // de buildPublishPayload; nada disto é recalculado aqui.
  const payload = await buildPublishPayload(product);
  const { title, descriptionHtml, retailPrice } = payload;
  ...
  return {
    ...
    resolvedFranchise: payload.resolvedFranchise,
    /** Line resolvida (sub-divisão do universo — ENTREGA 2). Alimenta alterpop.line no
     *  publish. NULL se o produto não pertence a nenhuma Line. */
    resolvedLine: payload.resolvedLine,
    /** Formato do produto (Tarefa 34/Decisão 18) — Figure, Keychain, Backpack, …
     *  Alimenta alterpop.format no publish. NULL se nenhuma categoria bateu. */
    resolvedFormat: payload.resolvedFormat,
    /** B6 — linha do fabricante (Ichibansho, Grandista, …). Alimenta
     *  alterpop.manufacturer_line no publish. NULL se o fabricante não estiver no mapa
     *  fechado ou nenhuma linha bater. */
    resolvedManufacturerLine: payload.resolvedManufacturerLine,
