...
export function mapCatalogProductToShopifyPayload(product, opts = {}) {
  ...
  const isValidImg = rawImg && /^https?:\/\/[^\s]+/i.test(rawImg);
  const cleanImgUrl = isValidImg ? encodeURI(rawImg) : null;
  ...
  return {
    ...
    restProduct: {
      product: {
        ...
        images: cleanImgUrl ? [{ src: cleanImgUrl }] : [],
      },
    },
    ...
    graphql: {
      ...
      media: cleanImgUrl
        ? [{ originalSource: cleanImgUrl, mediaContentType: "IMAGE" }]
        : [],
    },
  };
}
