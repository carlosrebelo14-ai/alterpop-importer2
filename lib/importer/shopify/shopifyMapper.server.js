 ...
 */
export function resolveShopifyCategoryGid(productType) {
...
export function buildProductDescriptionHtml(product) {
  ...
  // Dimensões — xml_info_dimensiones, presente em ~69% do catálogo. Ausente não gera
  // linha (nada de células vazias na tabela pública).
  const dimensions = String(product.dimensions || "").trim();
  if (dimensions) {
    specRows.push(`<tr>
      <th style="text-align:left;padding:10px 14px;background:#f8f9fa;border:1px solid #e9ecef;font-weight:600;color:#212529;">Dimensions (W × H × D)</th>
...
export async function mapCatalogProductToShopifyPayload(product, opts = {}) {
  ...
  const productType = resolveShopifyProductType(product.categoryMain);
  const shopifyCategoryGid = resolveShopifyCategoryGid(productType);
  ...
  return {
    ...
    barcode,
    weightKg,
    /** dimensões formatadas ("25 × 5 × 15 cm") ou null — alimenta ociostock.dimensions */
