...
export function buildProductDescriptionHtml(product) {
  ...
  // License / Universe — `franchises` é a árvore de categorias em bruto do
  // fornecedor (ver franchiseCatalogList.js), não nomes de coleção reais; um link
  // direto para /collections/<slug> caía quase sempre em 404. Usa o mesmo padrão
  // seguro já usado para marca/categoria acima: filtro por tag, que funciona
  // sempre porque não depende de nenhuma coleção ter sido criada para o valor.
  if (cleanFranchises.length > 0) {
    const franchiseLinks = cleanFranchises.map((f) => {
      const url = `/collections/all?filter.p.tag=${encodeURIComponent(f)}`;
