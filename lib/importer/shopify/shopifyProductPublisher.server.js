...
async function updateExistingProduct(client, mapped, variant, metafieldReady, ctx) {
  ...
  if (metafieldReady) {
    await setNetPriceMetafield(client, variant.product.id, mapped.netPrice);
    await setDimensionsMetafield(client, variant.product.id, mapped.dimensions);
  }
...
async function createNewProduct(client, mapped, metafieldReady, ctx) {
  ...
  if (metafieldReady) {
    await setNetPriceMetafield(client, created.id, mapped.netPrice);
    await setDimensionsMetafield(client, created.id, mapped.dimensions);
  }
