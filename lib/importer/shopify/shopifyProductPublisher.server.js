 ...
 * fornecedor. Muda `mapped` em memória, antes de qualquer chamada GraphQL — nunca
 * toca no `category` oficial da taxonomia (só no productType de texto livre), para
 * não arriscar mandar um GID inválido para a Shopify.
 ...
 * @param {string} productId
 */
async function isProductSyncLocked(client, productId) {
  try {
...
async function updateExistingProduct(client, mapped, variant, metafieldReady, ctx) {
  ...
  const locked = await isProductSyncLocked(client, variant.product.id);
  if (locked) {
