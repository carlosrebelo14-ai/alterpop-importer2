 ...
 * @param {ReturnType<typeof mapCatalogProductToShopifyPayload>} mapped
 */
 ...
 *   mapped: ReturnType<typeof mapCatalogProductToShopifyPayload>,
 *   locationId?: string,
 * }} params
 */
async function pushInventoryAfterPublish(client, params) {
  ...
  const result = await syncCatalogStockToShopify(client, {
    ...
    locationId,
    referenceDocumentUri: `gid://alterpop-importer/Publish/${mapped.sku}`,
  });
...
/**
 * @param {import('../shopifyClient.js').ShopifyClient} client
 * @param {import('../types.js').ProductRecord | object} catalogRow
 * @param {{
 ...
 * }} [ctx]
 */
export async function publishCatalogProductToShopify(client, catalogRow, ctx = {}) {
  const cache = ctx.variantCache || new Map();
  let metafieldReady = ctx.metafieldReady;
  ...
  const mapped = mapCatalogProductToShopifyPayload(catalogRow, { status: "ACTIVE" });
...
async function attachMedia(client, productId, mapped) {
  ...
  try {
    ...
    if (/image|media|url|invalid/i.test(msg)) {
