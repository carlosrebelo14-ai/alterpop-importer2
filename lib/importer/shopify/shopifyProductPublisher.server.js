...
export async function publishCatalogProductToShopify(client, catalogRow, opts = {}) {
  ...
  if (existing?.product?.id) {
    result = await updateExistingProduct(client, mapped, existing, metafieldReady, ctx);
  } else {
    result = await createNewProduct(client, mapped, metafieldReady, ctx);
  }

  // B18 item 5 (17/09/2026) — só grava lastPublishedTitle quando o título foi mesmo
  // escrito agora (nunca em produto sync_locked: aí o título na Shopify pode já não
  // corresponder ao que temos aqui, e marcá-lo como "publicado por nós" enganaria o
  // live-title-reconcile.js a tratar uma futura edição do Carlos como sendo nossa).
  if (!result.locked) {
    await recordLastPublishedTitle(catalogRow, mapped.title);
  }
...
/**
 * @param {{ shop?: string, sku?: string }} catalogRow
 * @param {string} title
 */
async function recordLastPublishedTitle(catalogRow, title) {
  if (!catalogRow?.shop || !catalogRow?.sku) return;
  await safePrisma(
    "shopifyPublish.lastPublishedTitle",
    () =>
      prisma.catalogProduct.update({
        where: { shop_sku: { shop: catalogRow.shop, sku: catalogRow.sku } },
        data: { lastPublishedTitle: title },
      }),
    { rethrow: false }
  );
}
...
async function setProductMetafieldsSafe(client, productId, mapped) {
  ...
  try {
    await setFranchiseMetafield(client, productId, mapped.resolvedFranchise);
  } catch (err) {
    console.warn(`[shopify-publish] metafield alterpop.franchise falhou para ${sku}:`, err?.message || err);
  }
  try {
    await setLineMetafield(client, productId, mapped.resolvedLine);
  } catch (err) {
    console.warn(`[shopify-publish] metafield alterpop.line falhou para ${sku}:`, err?.message || err);
  }
  try {
    await setFormatMetafield(client, productId, mapped.resolvedFormat);
  } catch (err) {
    console.warn(`[shopify-publish] metafield alterpop.format falhou para ${sku}:`, err?.message || err);
  }
  try {
    await setManufacturerLineMetafield(client, productId, mapped.resolvedManufacturerLine);
  } catch (err) {
    console.warn(`[shopify-publish] metafield alterpop.manufacturer_line falhou para ${sku}:`, err?.message || err);
  }
}

/**
 * Universo resolvido (Fase 4) → metafield `alterpop.franchise` (list.single_line_text_field).
 * Fecha a Fase 6b: cada produto publicado a partir daqui nasce com franquia e entra na
 * coleção Universe do tema. Sem universo resolvido → não grava nada (o tema distingue
 * campo ausente de vazio, e ausente é o correto).
 * @param {import('../shopifyClient.js').ShopifyClient} client
 * @param {string} productId
 * @param {string|null|undefined} universeName  nome canónico da tabela dos 41
 */
async function setFranchiseMetafield(client, productId, universeName) {
  // NFC: o valor tem de bater por code point com a condição EQUALS da coleção Universe
  // (ENTREGA 2 §4). "Pokémon" fica U+00E9, nunca e + U+0301.
  const name = String(universeName || "").trim().normalize("NFC");
  if (!name) return;

  // Tarefa 30 (Decisão 15) — rejeição no publisher: um universo DORMENTE não tem
  // coleção nenhuma na loja (a guarda tri-direcional garante isso antes do publish
  // sequer arrancar). Escrever o metafield na mesma deixava o produto com uma franquia
  // que nenhuma sala apanha — silenciosamente invisível. Não escreve nada; fica sem
  // alterpop.franchise até o universo cruzar o limiar e ganhar coleção.
  const universe = getUniverseByName(name);
  if (universe?.dormant) {
    console.warn(`[shopify-publish] alterpop.franchise rejeitado: "${name}" é dormente (sem coleção) — produto ${productId} fica sem o metafield`);
    return;
  }
...
/**
 * Line resolvida (ENTREGA 2) → metafield `alterpop.line` (list.single_line_text_field).
 * Sub-divisão dentro do universo (ex.: "The Mandalorian" dentro de "Star Wars"). O
 * produto continua a levar alterpop.franchise = "Star Wars"; este campo é adicional e
 * alimenta a coleção Line (templateSuffix "line"). Sem Line → não grava nada.
 * @param {import('../shopifyClient.js').ShopifyClient} client
 * @param {string} productId
 * @param {string|null|undefined} lineName  nome canónico da tabela de Lines
 */
async function setLineMetafield(client, productId, lineName) {
  const name = String(lineName || "").trim().normalize("NFC");
  if (!name) return;
...
/**
 * Tarefa 34/35 (Decisão 18) — formato de produto (Figure, Keychain, Backpack, …) →
 * metafield `alterpop.format` (list.single_line_text_field). Mesma convenção das
 * outras: sem formato resolvido, não grava nada.
 * @param {import('../shopifyClient.js').ShopifyClient} client
 * @param {string} productId
 * @param {string|null|undefined} formatName
 */
async function setFormatMetafield(client, productId, formatName) {
  const name = String(formatName || "").trim().normalize("NFC");
  if (!name) return;
...
/**
 * B6 — linha do fabricante (Ichibansho, Grandista, …) → metafield
 * `alterpop.manufacturer_line` (single_line_text_field — escalar, não lista: o
 * extrator só devolve um valor). Mesma convenção das outras: sem linha resolvida, não
 * grava nada.
 * @param {import('../shopifyClient.js').ShopifyClient} client
 * @param {string} productId
 * @param {string|null|undefined} lineName
 */
async function setManufacturerLineMetafield(client, productId, lineName) {
  const name = String(lineName || "").trim().normalize("NFC");
  if (!name) return;
...
async function setLicenceMetafield(client, productId, licence) {
  // Normalizado (Title Case) para bater sempre certo com a condição EQUALS da smart
  // collection em autoCollections.server.js, independentemente do casing no CSV do
  // fornecedor (ver licenceLabel.js).
  const value = normalizeLicenceLabel(licence);
  if (!value) return;
