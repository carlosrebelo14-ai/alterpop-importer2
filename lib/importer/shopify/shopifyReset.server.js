...
import { deactivateAllSyncErrors } from "../sync/syncErrorLog.server.js";
...
/**
 * Repõe toda a fila local publicada/em-erro para `targetStatus` e limpa o vínculo
 * Shopify (shopifyProductId, publishedAt, sync errors).
 *
 * `targetStatus`:
 *  - "APPROVED" (default) — reset de emergência clássico: os produtos voltam ao pool de
 *    publicação automática e o próximo ciclo re-publica-os.
 *  - "PENDING" — devolve à curadoria: saem do pool de publicação, nada é re-publicado
 *    sem aprovação explícita. É o que a limpeza pré-inaugural de 2026-09 usa (a loja
 *    fica vazia até à seleção curada à mão — ver docs/PLANO-normalizacao-franquias.md).
 *
 * @param {{ targetStatus?: "APPROVED" | "PENDING" }} [opts]
 */
export async function revertAllPublishedInQueue(opts = {}) {
  const targetStatus = opts.targetStatus === "PENDING" ? "PENDING" : "APPROVED";
  const queue = await loadCurationQueue();
  let reverted = 0;
...
export async function runShopifyCatalogReset(session, opts = {}) {
  ...
  try {
    ...
    // Estado local reposto para republicação limpa — erros de sync de execuções
    // anteriores deixam de ser relevantes (mesma razão do hook em
    // indexingStream.server.js, ver comentário lá).
    await deactivateAllSyncErrors(shop).catch(() => {});
