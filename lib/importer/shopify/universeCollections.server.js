 ...
 *
 * descriptionHtml — B14 (briefing backend, 24/09/2026, secção 3). A coleção nasce sem
 * descrição (não passa o campo no collectionCreate); NUNCA escrever aqui um placeholder
 * interno como "Coleção Universe — produtos com..." ou "Coleção criada
 * automaticamente — ...", os dois padrões de collectionDescriptionPlaceholder.js. Copy
 * editorial fica a cargo do Carlos no Admin.
 */
import {
  FRANCHISE_UNIVERSES,
 ...
 *
 * Chama publishCollectionOnCreation (collectionPublication.server.js, B14 secção 6)
 * depois de cada collectionCreate bem-sucedido — a ligação vive AQUI, na criação, não no
 * trigger-sync, para cobrir também este script manual (universe-collections-plan.js,
 * Fase 7, `--create`). O trigger-sync (secção 8, collectionPublicationReconcileCycle.
 * server.js — V14) fica só com a reconciliação sobre coleções já existentes.
 *
 * checkCyclePreconditions corre UMA VEZ por chamada (não por coleção) — falha nos
 * scopes ou no publicationId salta a publicação de TODAS as coleções deste lote, mas
 * nunca a criação: `created` sai preenchido na mesma.
 * @param {import('../shopifyClient.js').ShopifyClient} client
 * @param {{ adoptHandles?: string[], publicationId?: string }} [opts]
 */
export async function createUniverseCollections(client, opts = {}) {
  ...
  let preconditions = { ok: false, reason: null };
  if (toCreate.length) {
    preconditions = await checkCyclePreconditions(client, publicationId);
    if (!preconditions.ok) {
      console.warn(`[universeCollections] publicação saltada neste lote: ${preconditions.reason}`);
    }
  }
  ...
  for (const u of toCreate) {
    ...
    if (preconditions.ok) {
      await publishCollectionOnCreation(client, { collection, publicationId });
    }
  }
