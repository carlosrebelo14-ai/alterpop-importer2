...
export async function deactivateAllSyncErrors(shop) {
  ...
  await pruneOldSyncErrors(shop);
}

/** Dias a manter erros já resolvidos (active=false) antes de os apagar. */
const SYNC_ERROR_RETENTION_DAYS = 90;

/**
 * Poda (code review 2026-08-13) — sem isto, SyncErrorLog crescia sem limite para
 * sempre, uma reindexação completa de cada vez. Só apaga erros já inativos
 * (resolvidos ou invalidados); o histórico recente e tudo o que ainda está activo
 * fica sempre disponível na página "Logs de Erro".
 * @param {string} shop
 */
async function pruneOldSyncErrors(shop) {
  const cutoff = new Date(Date.now() - SYNC_ERROR_RETENTION_DAYS * 24 * 60 * 60 * 1000);
  await safePrisma("syncErrorLog.prune", () =>
    prisma.syncErrorLog.deleteMany({
      where: { shop, active: false, createdAt: { lt: cutoff } },
    })
  );
}
