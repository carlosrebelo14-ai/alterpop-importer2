...
export function validateRecord(record, options = {}) {
  ...
  if (requirePrice) {
    ...
    // Auditoria 2026-09-22 — este check era incondicional, ao contrário do seu
    // espelho (netPrice, logo abaixo), que só falha quando o OUTRO preço também não
    // cobre. Um grossPrice=0 com netPrice válido (precio_bruto='0' no CSV, não raro)
    // rejeitava o registo inteiro mesmo `hasPrice` (abaixo) já o reconhecendo como
    // válido via netPrice, e mesmo ProductImporter.buildVariantBulkInput() já cair
    // para netPrice quando grossPrice<=0 — o registo era descartado à toa.
    if (gross != null && gross <= 0 && (net == null || net <= 0)) {
      errors.push("grossPrice must be > 0 when netPrice is missing or <= 0");
    }
