    ...
    const result = await syncCatalogWithProgress(shop, settings, {
      ...
      onProgress(stats) {
        if (stats.indexed % 1000 === 0) {
          ...
          console.log(`[worker-mem] rss=${Math.round(mem.rss/1024/1024)}MB heapTotal=${Math.round(mem.heapTotal/1024/1024)}MB heapUsed=${Math.round(mem.heapUsed/1024/1024)}MB ext=${Math.round(mem.external/1024/1024)}MB arr=${Math.round(mem.arrayBuffers/1024/1024)}MB (indexed=${stats.indexed})`);
        }
