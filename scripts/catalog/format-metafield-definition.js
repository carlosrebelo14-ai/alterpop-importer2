#!/usr/bin/env node
/**
 * Tarefa 35 (Decisão 18, depende da Tarefa 34) — cria a definição de metafield
 * `alterpop.format`.
 *
 * list.single_line_text_field, pinned, com capability smartCollectionCondition ligada.
 * Mesma forma que `alterpop.franchise`/`alterpop.line` — ver
 * lib/importer/shopify/franchiseMetafieldDefinition.js.
 *
 * Idempotente: se já existir, imprime o GID e não recria.
 *
 * Depois de correr, colar o GID devolvido em ALTERPOP_FORMAT_DEFINITION_GID
 * (franchiseMetafieldDefinition.js).
 *
 * Precisa de sessão OAuth offline (correr na Fly):
 *   node scripts/catalog/format-metafield-definition.js
 *   node scripts/catalog/format-metafield-definition.js --dry-run
 */
import { loadOfflineSessionForShop } from "../../lib/session/loadOfflineSessionForShop.server.js";
import { createShopifyClientFromSession } from "../../lib/importer/shopifyClient.js";
import { ALTERPOP_FORMAT_DEFINITION } from "../../lib/importer/shopify/franchiseMetafieldDefinition.js";
...
const FIND = `
  query FormatDefFind {
    metafieldDefinitions(first: 1, ownerType: PRODUCT, namespace: "alterpop", key: "format") {
...
const CREATE = `
  mutation FormatDefCreate($definition: MetafieldDefinitionInput!) {
...
async function main() {
  ...
  if (found?.id) {
    ...
    console.log(`\n→ ALTERPOP_FORMAT_DEFINITION_GID = "${found.id}"`);
    return;
  }

  const def = ALTERPOP_FORMAT_DEFINITION;
  const definition = {
  ...
  console.log(`\n→ ALTERPOP_FORMAT_DEFINITION_GID = "${created.id}"`);
}
