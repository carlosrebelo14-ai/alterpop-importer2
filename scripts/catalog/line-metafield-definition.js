#!/usr/bin/env node
/**
 * ENTREGA 2 · Tarefa 2 — cria a definição de metafield `alterpop.line`.
 *
 * list.single_line_text_field, pinned, com capability smartCollectionCondition ligada
 * (para a coleção Line poder usar a regra `alterpop.line EQUALS <nome>`). Mesma forma
 * que `alterpop.franchise` — ver lib/importer/shopify/franchiseMetafieldDefinition.js.
 *
 * Idempotente: se já existir, imprime o GID e não recria.
 *
 * Depois de correr, colar o GID devolvido em ALTERPOP_LINE_DEFINITION_GID
 * (franchiseMetafieldDefinition.js).
 *
 * Precisa de sessão OAuth offline (correr na Fly):
 *   node scripts/catalog/line-metafield-definition.js
 *   node scripts/catalog/line-metafield-definition.js --dry-run
 */
import { loadOfflineSessionForShop } from "../../lib/session/loadOfflineSessionForShop.server.js";
import { createShopifyClientFromSession } from "../../lib/importer/shopifyClient.js";
import { ALTERPOP_LINE_DEFINITION } from "../../lib/importer/shopify/franchiseMetafieldDefinition.js";
...
const FIND = `
  query LineDefFind {
    metafieldDefinitions(first: 1, ownerType: PRODUCT, namespace: "alterpop", key: "line") {
...
const CREATE = `
  mutation LineDefCreate($definition: MetafieldDefinitionInput!) {
...
async function main() {
  ...
  if (found?.id) {
    ...
    console.log(`\n→ ALTERPOP_LINE_DEFINITION_GID = "${found.id}"`);
    return;
  }

  const def = ALTERPOP_LINE_DEFINITION;
  const definition = {
  ...
  console.log(`\n→ ALTERPOP_LINE_DEFINITION_GID = "${created.id}"`);
}
